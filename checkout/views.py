...
from .models import CarItem, Order
...
class CartItemView(TemplateView):
    ...
    def get_formset(self, clear=False):
        CartItemFormSet = modelformset_factory(
            CarItem, fields=('quantity',), can_delete=True, extra=0
        )
        session_key = self.request.session.session_key
        if session_key:
            if clear:
                formset = CartItemFormSet(
                    queryset=CarItem.objects.filter(cart_key=session_key)
                )
            else:
                formset = CartItemFormSet(
                    queryset=CarItem.objects.filter(cart_key=session_key),
            ...
            formset = CartItemFormSet(queryset=CarItem.objects.none())
        return formset
...
class CheckoutView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if session_key and CarItem.objects.filter(cart_key=session_key).exists():
            cart_items = CarItem.objects.filter(cart_key=session_key)
...
class PagSeguroView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        pg.notification.url = self.request.build_absolute_uri(
...
def paypal_notification(sender, **kwargs):
    ...
    if ipn_obj.payment_status == ST_PP_COMPLETED and \
        ipn_obj.receiver_emaiil == settings.PAYPAL_EMAIL:
        try:
...
pag_seguro = PagSeguroView.as_view()
pay_pal = PaypalView.as_view()
